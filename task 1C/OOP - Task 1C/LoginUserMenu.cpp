...
void LoginUserMenu::OutputOptions()
{
	...
	{
		std::cout << "  " << (i + 1) << ") " << app->accounts.getListItem(app->accounts, 0)->users.getListItem(app->accounts.getListItem(app->accounts, 0)->users, i)->GetUsername() << "\n";
	}
...
bool LoginUserMenu::HandleChoice(char choice)
{
	...
	default: {
		...
		{
			std::string username = app->GetCurrentAccount()->users.getListItem(app->GetCurrentAccount()->users, index)->GetUsername();
